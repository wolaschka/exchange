# Source: https://qceqatwapp101.sd01.unicreditgroup.eu:5443/docs/import_agreements.html

# File Format for Agreements
...
When the import is run, the system checks to see whether the agreement already exists. This check is done using the [Source Reference](<#sourcereference>) and the Agreement Name as follows:
...
**Note:**
  ...
  2. It is only possible to import agreements that are for the OTC (except OTC Lock Up Only - see item 3), Repo and Securities Lending Business Lines. This is determined via Agreement Type Field.
  3. It is not possible to import OTC Lock Up Only agreements using this configuration. There is a separate [import configuration](<import_agreement_lock_up_only.md>) to process Lock Up Only agreements.
  4. Margin Valuation Frequency can be included on Agreement imports from TLM Collateral version 5.8 onwards. The Fields on the import are described below. As there are numerous permutations as to how Frequencies can be defined, some examples of the mapping file and import data are shown in examples of import [Agreement Margin Valuation Frequency](<example_import_agmt_margin_val_freq.md>).
...
The value on the data file must match a value defined in the database, otherwise [validation message 237](<validation_messages.md#237>) is raised and the **record is not imported**.  
...
The field is not mandatory but if mapped the value supplied must match a record in [Agreement Status Detail reference](<reference.md>) data otherwise [Validation Message 235](<validation_messages.md#235>) will be raised, and a Null Value used.  
...
For an insert, any trade types associated with the Agreement Type are automatically set to be on the "Included" side of the [Trade Types select](<agreements_tradecoverage.md>) dialog for the relevant agreement.  
...
2\. Once this field is set as True either via the GUI or by an import, it cannot be subsequently changed back to False by an import. This can only be achieved via the application's GUI. If an attempt to change an existing value to false via the import [valuation message 334](<validation_messages.md#334>) is raised.  
...
Lock Up Only (see notes)  
  
Both Lock Up And Variation | This value indicates whether the collateral that is **received from** the Counterparty on the Agreement can be rehypothecated.  
...
[validation message 9](<validation_messages.md#9>) will be raised, **and the whole record will not be loaded**.  
  
If the value is invalid, then [validation message 242](<validation_messages.md#242>) will be raised **and the whole record will not be loaded.**  
  
If the value is specified as **_Lock Up Only_** an additional validation takes place: If **_CollateralAllocationType_** is not defined as **_Segregate Lock Up And Variation_** , then [validation message 243](<validation_messages.md#243>) will be raised **and the whole record will not be loaded**.  
...
Collat Allocation | CollateralAllocationType | Yes | Char (50) | Auto Allocate  
  
Segregate Lock Up And Variation | This field is mandatory and denotes how collateral is to be allocated.  
  
It must be defined as one of the two values listed to the left.  
  
The very first validation on this import is to determine if a **Lock Up Only** Agreement is tried to be imported using this import configuration. If so, a [validation message 343](<validation_messages.md#343>) is raised and the entire record is not accepted.  
  
If an existing **Lock Up Only** agreement is tried to be changed to an OTC agreement via this import, a [validation message 351](<validation_messages.md#351>) is raised and the entire record is not accepted.  
  
**Otherwise** :  
  
If defined as **Auto Allocate** , collateral will be "auto allocated". This means the system will automatically allocate any collateral toward the lock up requirement first. Once the lock up requirement is met, any remaining collateral will be allocated toward the variation requirement.  
  
If defined as **Segregate Lock Up and Variation** , the system does not perform any such allocations, and collateral must be explicitly defined as being either for Lock Up margin type or variation margin type.  
  
Additionally, any surplus collateral held for either the lock up requirement is not applied to any deficit in the variation requirement, and vice-versa.  
  
If the field is missing, [validation message 9](<validation_messages.md#9>)  
  
is raised, **and the record is not imported**.  
  
If the field does not contain a permissible value, [validation message 240](<validation_messages.md#240>) is raised, **and the record is not imported**.  
  
An additional validation is performed on this field if it is set to Segregate Lock Up and Variation.  
  
If so, then LockUpCalculationType value must be defined as "Not Netted Against Variation" otherwise [validation message 241](<validation_messages.md#241>) will be raised and the record will not be imported.  
  
Please see  
  
[Changing Collateral Allocation Type on an OTC Agreement](<agreement_change_collatallocation.md>) for more information.  
...
If the quote type on the file doesn't match any of the above three values,   
  
[Validation Message 9](<validation_messages.md#9>) can be seen  
...
If the value is not supplied on the file,   
  
[Validation Message 9](<validation_messages.md#9>)  
  
will be seen and the existing quote type for the agreement will remain in the system.  
  
**Note:** This value cannot be amended to Auto Allocate for existing agreements that exist with value of Segregate Lock Up and Variation. If this is attempted, [validation message 249](<validation_messages.md#249>) will be raised, and the whole import record for this agreement will be ignored.  
...
**Note:** If value is not supplied, then the agreement will be set to use the system default value even if a specific value existed for the agreement before the import.  
  
Similarly, if a value is supplied on the file for price source that doesn't exist in the system, [Validation Message 196](<validation_messages.md#196>) can be seen and the price source for the agreement will be set to use system default, even if it was previously set to a defined price source.  
...
The Counterparty name cannot be changed if the agreement already has [branches](<agreements_tradecoverage.md>) or [contacts](<contactinfo.md#adding-new-contacts>) associated to it.  
...
Although not compulsory, if not included in the file, or the value is invalid, [validation message 251](<validation_messages.md#251>) will be raised, and the system default value will be applied (value per the web.config file) If no system default is defined, the value will be defaulted to T+1.  
Exclude trades ending on or before | ExcludesTradeDays | No | Char (255) | T+2 | The Trades Filter option for Repo agreements to determine the basis on which trades should be excluded from the Exposure Calculation based on their end date. The allowed choices are:  
...
**List** |  | Extension Values can be loaded on the Agreement Import file.  
...
(See [Example of Agreement Mapping File](<import_example_agmt_mapping_file.md>) with Extensions for examples)  
...
Calendars | HolidayCalendar | No | Char (2000) | UK Holidays | This is the name of the Holiday Calendar reference data item that will be associated to the agreement.  
...
If more than one Holiday Calendar on an agreement is being imported, use a [sub delimiter in the mapping file](<import_using_sub_delimiter.md>).  
...
need to be populated. | If this field is not supplied, the system will do the following:- If an entry for this already exists for the agreement, the system will disregard the value on the file.  
...
**Note:** If an invalid date is specified, the system will raise [validation message 223](<validation_messages.md#223>), and the system will default interest Payment period to be Monthly, last calendar day of the month, unless exact dates already exist in the database.  
...
**Note:** If values are supplied for this field, then any existing exact dates in the system will be replaced with those specified in the import file.  
...
**_(NB - this warning is currently not working on weekly timings)_**  
...
If not, the record will not be loaded.  
Include trades starting on or before | IncludesTradeDays | No | Char (255) | T+1 | The [Trades Filter](<agreements_define.md#tradesfilters>) option for Repo agreements to determine the basis on which trades should be included from the Exposure Calculation based on their start date. The allowed choices are:  
...
Lock Up Calculation Type | LockUpCalculationType | No | Char (50) | NettedAgainstVariation | This field indicates how any Lock Up Margin will be treated in the Margin Calculation. The allowed values are:  
...
If no value is provided (or an invalid value is provided) the default value " NotNetted against variation" will be used.  
  
A validation warning will be provided when an invalid value is supplied.  
  
[Validation message number 149](<validation_messages.md#149>)  
  
For further information about the [Lock Up Calculation Type](<agreements_general.md#lockupcalc>) see [Additional and Lock Up Margins](<agmtmargins_otc.md#OTCAgmtMargins>).  
  
NB. This field works in conjunction with Collateral Allocation Type. That is, if this field is defined as Netted Against Variation, then Collateral Allocation Type cannot be defined as Segregate Lock Up and Variation.  
Date (in Master Agreement Section) | MasterAgreementDate | No | Date | 13-Sep-2009 | The date the Master Agreement was signed.  
  
Value if mapped must be in a recognizable date format otherwise [validation message 10](<validation_messages.md#10>) will be raises and a Null value will be used.  
Specific City | NotificationCity | No | Char(100) | USD | The city or Counterparty Managing Location.  
  
An invalid city or Managing Location was specificied. The value is set to Null, [validation message 405](<validation_messages.md#405>).  
  
Notification Time is required when a Notification City is specified,[validation message 407](<validation_messages.md#407>).  
[Notification Time](<agreements_marginterms.md#notificationtime>) | NotificationTime | No | Char(25) | 20:24 | This field is used to specify a time for the notification.  
  
Time must be specified in a 24 hour time format, [validation message 406](<validation_messages.md#406>).  
  
A city or a Counterparty Managing Location is required when a time is specified, [validation message 408](<validation_messages.md#408>).  
...
If the field is mapped, but an invalid value is included, then [validation message 19](<validation_messages.md#19>) and [validation message 9](<validation_messages.md#9>) will be raised, and the record will not be imported.  
  
If the field is mapped, but a value is not present on the file, then the [validation message 9](<validation_messages.md#9>) will be raised, and the record will not be imported.  
...
Price Adjustment (only seen on Repo and Securities Lending Agreements) | PriceAdjustment | Yes (for Repo and Securities Lending Agreements) | Char (50) | Permissible values:  
...
Value if mapped must be in a recognizable date format otherwise [validation message 10](<validation_messages.md#10>) will be raised and a Null value will be used.  
...
If Method is defined as Flat, then the system will compare the _numeric_ difference between the Principal Call Amount and the Counterparty Call Amount to the SplitToleranceMethod and evaluate whether to incorporate splitting in the calculation of Agreed amounts  
...
Tri-Party Custodian Lock Up Collateral (advanced tab) | TriPartyLockupCustodian | No | var (255) | Clearstream | The Tri-Party Custodian that the Agreement's lock up margin movements settle through.  
...
(Margin Terms tab) | UseMTAOnLockUpMarginTerms | Yes | Bit | Any of the following values can be loaded: (case insensitive)  
...
0 | This field is mandatory, and if missing or an invalid value will raise [validation message 9](<validation_messages.md#9>) **and the whole record will not be loaded**.  
  
Additionally, it is only permissible to set as **True** if the agreement is an OTC Agreement AND has a collateral allocation method of Segregate Lock Up and Variation.  
  
If the value is set as True for a Non OTC Agreement, [validation message 256](<validation_messages.md#256>) will be raised, and the value will be set as false.  
  
Similarly, if the value is to set as True for an OTC Agreement with collateral allocation method of Auto Allocate, [validation message 257](<validation_messages.md#257>) will be raised, and the value will be set as false.  
Apply Lock Up to Rounding  
  
(MarginTerms Tab) | UseRoundingOnLockUpMarginTerms | Yes | Bit | Any of the following values can be loaded: (case insensitive)  
...
Similarly, if the value is set as True for an OTC Agreement with collateral allocation method of Auto Allocate, [validation message 257](<validation_messages.md#257>) will be raised, and the value will be set as false.  
...
Margin Valuation Frequency | ValuationFreq | No - but see notes | Char (25) | Acceptable Values  
...
\- If value = Exact,ValuationFreqExactDates needs to be populated.  
  
\- If Value = Weekly, ValuationFreqDayOfWeek  
needs to be populated.  
...
Additionally, depending on ValuationFreqCalendarType  
  
definition, either  
  
ValuationFreqDayOfMonth  
  
or  
  
ValuationFreqWeekOfMonth  
  
need to be populated. | If this field is not supplied, the system will do the following:  
  
If an entry for this already exists for the agreement, it will be overwritten if the value is valid.  
  
\- If the import is inserting a new agreement - the system will set the value for the agreement to be Daily.  
  
In either case, if the value is invalid, it will ignore the value defined for any of these fields:  
  
\- ValuationFreqExactDates  
\- ValuationFreqDayOfWeek  
\- ValuationFreqDayOfMonth  
\- ValuationFreqCalendarType  
\- ValuationFreqMonths  
\- ValuationFreqWeekOfMonth  
  
If the value supplied is invalid, and the import is inserting a new agreement, the agreement will be created with a default value of Daily and [validation message 401](<validation_messages.md#401>) will be raised.  
  
If the import is amending a record and the value is incorrect then the existing record will be untouched, but the same validation message will be raised.  
  
It is therefore recommended that the field be populated.  
  
**Note:** When the Valuation Frequency is set to Weekly or Monthly, the Starting Date will always default to the import date and the Business Day Adjustment will always default to Next.  
Margin Valuation Frequency Exact Dates | ValuationFreqExactDates | No - but see notes | Char (1000) | 27/10/2020 | When ValuationFreqExactDate is defined as Exact, record in this field the exact dates for the valuation, using the date format specified in the agreement configuration.  
  
Numerous dates can be specified using a sub-delimiter.  
  
**Note:** If an invalid date is specified, the system will raise [validation message 409](<validation_messages.md#409>) and the system will default Valuation Frequency period to be Daily if this a new agreement. If this is an amendment to the agreement, the value in the database will remain the same.  
  
If duplicate dates are supplied in this field for the same agreement, [validation message 410](<validation_messages.md#410>) is raised, and one of the duplicates is loaded.  
  
**Note:** If values are supplied for this field, that any existing exact dates in the system will be replaced with those specified in the import file.  
Margin Valuation Frequency - weekly selection | ValuationFreqDayOfWeek | No - but see notes | Char (25) | Monday  
...
Sunday | Although not mandatory, this field needs to be populated when Margin Valuation Frequency is defined as Weekly or Monthly and Valuation Frequency Calendar Type = Relative Day.  
  
If this value is not supplied then the system will raise [validation message 403](<validation_messages.md#403>)  
  
If the import is creating an agreement (insert) then if missing the system will default the valuation frequency to be Daily.  
  
If the import is amending an existing agreement (update), and this field is blank, then the existing day of week in database will be unchanged.  
  
If an invalid day of week is included in the import file, [validation message 404](<validation_messages.md#404>) is raised. If an existing day of the week exists for the agreement in the system this will be untouched.  
  
**Note:** Although importable values [Monday → Sunday], only values of Monday → Friday are selectable via the GUI for Monthly Relative Day frequencies.  
Valuation Frequency - monthly selection | ValuationFreqDayOfMonth | No - but see notes | Char (25) | 1st, 2nd, 3rd...Last  
  
If ValuationFrequencyCalendarType = Calendar Day values can be 1st, 2nd...31st or Last  
  
ValuationFrequencyCalendarType = Business Day values can be 1st, 2nd...23rd or Last | When Valuation Frequency is defined as Monthly and ValuationFrequencyCalendarType is defined as either CalendarDay or BusinessDay, this field needs to be populated.  
  
If an incorrect value is specified - e.g. 12st or no value is included, then [validation message 402](<validation_messages.md#402>) is raised and newly inserted agreements will be created with a default value of Daily.  
  
If the import is amending a record and the value is incorrect then the existing record will be untouched, but the same validation message will be raised.  
Valuation Frequency - monthly selection | ValuationFreqCalendarType | No - but see notes | Char (25) | Calendar Day  
  
Business Day  
  
Relative Day | When Valuation Frequency is defined as Monthly this field needs to be populated.  
  
If it is incorrectly populated, newly inserted agreements will have the Valuation Frequency set to Daily and receive [validation message 402](<validation_messages.md#402>).  
  
If the import is amending a record and the value is incorrect then the existing record will be untouched, but the same validation message will be raised.  
Valuation Frequency - monthly selection | ValuationFreqMonths | No - but see notes | Char (500) | January  
  
February...December  
  
Alternatively - ALL will populate all months. | When Valuation Frequency is defined as Monthly this field needs to be populated.  
  
Specific Months can be defined by specifying a sub delimiter in the config file.  
  
Alternatively, including ALL as the value for this will populate all months.  
  
If not month is specified, or an invalid month is specified, [validation message 402](<validation_messages.md#402>) will be raised and newly inserted agreements will be defaulted to be Valuation Frequency of Daily.  
  
If the import is amending a record and the value is incorrect then the existing record will be untouched, but the same validation message will be raised.  
Valuation Frequency - monthly selection | ValuationFreqWeekOfMonth | No - but see notes | Char (25) | First  
...
Last | When Valuation Frequency is defined as Monthly and Valuation Frequency Calendar Type is defined as Relative Day this field needs to be populated to define which week within the month that the Valuation Frequency will occur.  
  
If the value is omitted or an incorrect value is supplied.  
  
[Validation message 402](<validation_messages.md#402>) will be raised, and newly inserted agreements will be defaulted to be Valuation Frequency of Daily.  
  
If the import is amending a record and the value is incorrect then the existing record will be untouched, but the same validation message will be raised.  
  
**Note:** When agreements are inserted as a result of an import the "new trade type behavior" field on the Trade Types tab is set to excluded by default. There are no checks done on the import to see whether an agreement already exists for the same Principal, Counterparty and Trade Type combination. If agreements exist with overlapping Trade Type combinations for the same Principal and Counterparty, problems will could occur when running the Trade Assignment task, as there may be trades that could be assigned to more than one agreement. In this case, relevant validation messages would indicate the issues after running the Trade Assignment Task.

As not all of the agreement data can be inserted using an import it may be necessary to review new agreements in order that any remaining data can be completed manually. Newly created agreements can be found by using the "Created Since" filtering option on the agreement screen.
