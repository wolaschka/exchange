# Source: https://qceqatwapp101.sd01.unicreditgroup.eu:5443/docs/upcomingcoupons.html

# Upcoming Coupons

## Overview

The majority of debt Instruments (bonds) that are provided as collateral to satisfy margin requirements pay a coupon (interest) at predefined frequencies (typically twice a year) to the bond owner. The date on which the coupon is due to be paid is known as “Coupon Date”.

The establishment of whom the bond owner is for the purposes of paying the coupon on coupon date is determined on a date known as "Record date". Record date is a date prior to the coupon payment date. Whoever owns the bond at close of trading on record date will receive the upcoming coupon payment on coupon date irrespective of who is actually holding the bond on that date.

The "ex-date" is the date on which the seller, and not the buyer, of an instrument will be entitled to a recently announced coupon. The ex-date is usually two business days before the record date.

The payment of the coupon can have ramifications for Collateral Practitioners:

  * The coupon payment will affect the value of collateral.
  * There are operational processes involved in processing the coupon.
  * There may be tax implications of being the bond holder at record date or coupon date.



Many Collateral Practitioners therefore wish to know in advance that bonds in their Collateral Inventory, whether held or posted, are due to pay coupons. They can then best decide how they wish to process the upcoming coupon event.

The Inventory Management Module provides functionality to deal with upcoming coupons:

  * Collateral Positions in Bonds that are due to pay a coupon can be identified.
  * [Substitutions](<#create-substitution>) can be created for positions in those bonds that are due to pay a coupon.



## Search Criteria

To identify upcoming coupons on Collateral Positions, filters can be applied to return a subset of specific data within the following parameters:
...
Coupon Date | This is the date on which the debt instrument coupon is paid. If this is the date that should be included in the filter criteria, then click the radial button.  
Ex-Date | This is the date on which the seller, and not the buyer, of an instrument will be entitled to a recently announced coupon. If this is the date that should be included in the filter criteria, then click the radial button.  
...
The grid will populate with held and/or posted Collateral Positions per Agreement in any bonds which match the above search criteria within the inclusive date range specified.

By default, the grid contents will be sorted by:

  * Coupon Date (Ascending Order)
...
**LockUp:** collateral positions of this type should only exist for [OTC Agreements that separate Lock Up and variation requirements](<agreements_general.md#collatallocation>) or for [OTC Lock Up Only agreements](<agreements_define.md#otc-lock-up-only>). Where this is the case, collateral of this type is only ever allocated toward lock up margin requirements.  
...
Instrument Ref | The Primary ID of the Instrument with the upcoming coupon. By clicking on the [instrument](<instruments_debt.md#general>), a new tab in the browser will appear landing on the page with the instrument's details where the user can view or edit the details.  
...
Some Collateral Practitioners may elect to deal with upcoming coupons by substituting the affected bond(s) prior to the Record Date. TLM® Collateral Management allows for substitutions to be created for positions in securities where the coupon is upcoming.

The substitutions can be created directly from the Upcoming Coupons screen. If they are created directly from this screen, the position data in the grid is used to populate the return leg of the Substitution.
...
**Note the following:**
     ...
     * Notional to be substituted is pre-populated with the Available Notional amount.
     * Reason for substitution is pre-populated with "Upcoming Coupon."
     * Settlement Date is defaulted as per the configured debt instrument offset for the system.
     * The collateral movements created by the substitution will be for the same Margin Type as the collateral position selected.
...
Any of the first three values can be amended if required, but will be subject to validation. It is not possible to amend the margin type on the collateral movements created by the substitution.
  ...
  5. The Substitution is created, and placed in the _Substitutions Workflow - Pending Substitutions_ sate, with the return collateral movement.
