...
Approvals requirement is not mandatory. It can be [enabled / disabled](<system_configuration.md#database-approvals>) from an external configuration setting.
...
There are a few exceptions whereby certain Agreement data is treated separately from the Agreement and has its own approval process. That is, it needs to be approved separately from the Agreement. Henceforward, these items will be referred to as "**Agreement _Related_ Data Items**". These exceptions are:
  ...
  * [Standard Settlement Instructions](<agreement_ssi_approvals.md>)
...
When Approvals is [enabled](<system_configuration.md#database-approvals>), the following behavior occurs:
...
10 | Details of the Agreement's Pending Approval changes are placed into the [Pending screen](<approvals.md#pending>). There will be one record for all Agreement Data Items and separate records for the Agreement Related Data Items.  
11 | Clicking on the record in [Pending screen](<approvals.md#pending>) will allow a user to see the Agreement's pending changes on a field by field "before" and "after" basis.  
...
14 | [Agreement Data Items are validated](<agreement_valid_approvals.md>) when they are approved to ensure that the data being approved is still valid.  
15 | When Pending values are approved, a record of the change, together with the users whom updated and approved the values is created in [audit](<audit.md>).
