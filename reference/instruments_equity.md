# Source: https://qceqatwapp101.sd01.unicreditgroup.eu:5443/docs/instruments_equity.html

# Equity Instruments

Instrument data can be manually entered into the Instruments screen or loaded into the system from a file. Data which is entered manually will be overwritten if the same Instrument is also loaded from a file.

When an equity instrument is viewed, created or edited, it will contain 6 tabs of information. Each tab is described below.
...
| Stock Index | The Index that the equity instrument is classified under.  
| Exchange | Principal exchange on which the instrument is traded. For example, London Stock Exchange.  
...
### Add/Delete Instrument Ratings
  ...
  * **Delete** \- To remove data from the grid, click on the ![delete_icon.jpg](Images/delete_icon.jpg) icon located next the row that needs to be deleted. As this action is not reversible, the user will be prompted with a warning message asking to confirm the deletion or cancel the action. The user can click **Cancel** or **Delete** c.
...
Section | Field | Description  
---|---|---  
**Past Performance** | 52 Week Low | The highest price for the Instrument within the last 52 weeks.  
| 52 Week High | The lowest price for the Instrument within the last 52 weeks.  
**Prices Filter** | Price Quote | Denotes what type of price the numeric value represents. (MID/BID/ASK)  
**Prices** | Source |   
| Quote | This denotes what type of price the numeric value represents. (MID/BID/ASK)  
| Price |   
| Bid/Ask Spread | The spread between the Bid and Ask Price.  
| Calculated | Denotes whether Bid/Ask Spread was calculated during price import.  
| Date | The date of the price. This field must be specified if the prices are imported. If manually added, this field will default to today.  
...
Section | Field | Description  
---|---|---  
**Issuer Details** | Issuer Domicile | The domicile of the issuer.  
| Issuer Type | The type of the issuer.  
| Issuer Industry Sector | The Industry Sector of the Issuer.  
| Issuer Industry Group | The Industry Group of Issuer.  
| Collateral Type | The collateral type associated to the Instrument.  
  
## Market Data

Section | Field | Description  
---|---|---  
**Shares Characteristics** | Issue Date | The date that the instrument was issued.  
| Shares Issued | Number of shares originally issued.  
| Shares Outstanding | The total number of shares held by the public plus any restricted shares held by officers and insiders at a company.  
| Institutional Ownership (%) | The percentage of shares owned by Institutions. The rest is owned by individuals and directors (non-institutional).  
| Is Optionable | Whether options are available on the Instrument.  
**Risk, Liquidity, and Value Indicators** | Market Capitalization | Market capitalization (often market cap) is a measurement of the size of a business enterprise (corporation) equal to the share price times the number of shares outstanding.  
| Trading Volume | Average or daily trading volume.  
| Average Volatility | The annualized standard deviation of an equity instrument’s closing price over the previous year. A higher value indicates larger changes in the instrument price, where a lower value indicates smaller price changes.  
| Beta | A measure of an Instrument's volatility relative to a benchmark, e.g. the S&P 500.  
| UCITS Compliant | UCITS stands for Undertakings for Collective Investments in Transferable Securities and is a regulator within the European Union.  
| P/E Ratio | The P/E Ratio shows how much investors are willing to pay per currency unit of a company's earnings. Higher future earnings growth is expected when a company has a high P/E ratio compared to companies with lower P/E ratios.  
  
## Dividends

Field | Description  
---|---  
Dividend Payable | The amount of dividend per share.  
Dividend Currency | The currency of the Dividend.  
Ex-Date | Ex-Date is the first day that an Equity Instrument trades without entitlement to the next dividend. The ex-date is typically two business days prior to the record date which companies use to determine who should receive the dividend.  
Record Date | Date on which a shareholder must officially own shares in order to be entitled to a dividend. Also applicable to coupons on debt instrument.  
Next Dividend Date | The Next Date for Dividend Payment.  
